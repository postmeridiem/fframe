...
class TargetState {
  ...
  factory TargetState.fromUri(NavigationNotifier navigationNotifier, Uri uri) {
    ...
    Console.log("Routing to ${targetState.navigationTarget.path}", scope: "fframeLog.TargetState.targetState", level: LogLevel.fframe);
    return targetState;
  }
  ...
  factory TargetState.defaultRoute() {
    if (navigationNotifier.navigationConfig.navigationTargets.isEmpty && navigationNotifier.navigationConfig.navigationTargets.isNotEmpty) {
      ...
      Console.log("There are no unauthenticated routes", scope: "TargetState.defaultRoute", level: LogLevel.fframe);
      if (navigationNotifier.pendingAuth == false && navigationNotifier.isSignedIn != true) {
        ...
        Console.log("No applicable routes within the access control. Route to the signin page", scope: "TargetState.defaultRoute", level: LogLevel.dev);
        return TargetState(
      ...
      Console.log("Awaiting auth,  route to the signin page or wait page", scope: "TargetState.defaultRoute", level: LogLevel.fframe);
      return TargetState(
    ...
    if (targetState.navigationTarget.navigationTabs?.isNotEmpty == true) {
      Console.log("Route to the first available tab ${targetState.navigationTarget.navigationTabs!.first.title}", scope: "fframeLog.TargetState.defaultRoute", level: LogLevel.fframe);
      targetState = TargetState(navigationTarget: targetState.navigationTarget.navigationTabs!.first);
    }

    if (navigationNotifier.nextState.isNotEmpty) {
      Console.log("Route to ${navigationNotifier.nextState.first.targetState.navigationTarget.title} at ${navigationNotifier.nextState.first.targetState.navigationTarget.path}", scope: "fframeLog.TargetState.defaultRoute", level: LogLevel.fframe);
