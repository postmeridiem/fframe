...
import 'package:fframe/views/user/user_form.dart';
...
class UserDocument extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Row(
      children: [
        Flexible(
          child: MainCanvas(context, user, documentReference),
        ),
        SizedBox(
          width: 250,
          child: ContextCanvas(context, user),
        ),
      ],
    );
  }
...
class MainCanvas extends StatelessWidget {
  const MainCanvas(BuildContext context, this.user, this.documentReference);
  final User user;
  final DocumentReference documentReference;
        ...
        body: NestedScrollView(
          headerSliverBuilder: (BuildContext context, bool innerBoxIsScrolled) {
            return <Widget>[
              new SliverAppBar(
                primary: false,
                title: DocumentTitle(context, user.name!),
                floating: true,
                pinned: false,
                snap: true,
...
class ContextCanvas extends StatelessWidget {
  const ContextCanvas(BuildContext context, this.user);
  final User user;
