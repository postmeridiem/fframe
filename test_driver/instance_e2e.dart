...
void runInstanceTests() {
  group('FirebaseAuth.instance', () {
    ...
    group('authStateChanges()', () {
      ...
      test('calls callback with the current user and when auth state changes', () async {
    ...
    group('userChanges()', () {
      ...
      test('fires once on first initialization of FirebaseAuth', () async {
        ...
        Stream<User?> stream = FirebaseAuth.instanceFor(app: second).userChanges();
      ...
      test('calls callback with the current user and when user state changes', () async {
        await ensureSignedIn(testEmail);
    ...
    group('confirmPasswordReset()', () {
      test('throws on invalid code', () async {
        try {
          await FirebaseAuth.instance.confirmPasswordReset(code: '!!!!!!', newPassword: 'thingamajig');
          fail('Should have thrown');
        } on FirebaseException catch (e) {
          expect(e.code, equals('invalid-action-code'));
        } catch (e) {
    ...
    group('createUserWithEmailAndPassword', () {
      test('should create a user with an email and password', () async {
        ...
        await FirebaseAuth.instance.createUserWithEmailAndPassword(email: email, password: testPassword).then(successCallback as Function(UserCredential));
      });

      test('fails if creating a user which already exists', () async {
        await ensureSignedIn(testEmail);
        try {
          await FirebaseAuth.instance.createUserWithEmailAndPassword(email: testEmail, password: '123456');
      ...
      test('fails if creating a user with an invalid email', () async {
        await ensureSignedIn(testEmail);
        try {
          await FirebaseAuth.instance.createUserWithEmailAndPassword(email: '!!!!!', password: '123456');
          fail('Should have thrown FirebaseAuthException');
        } on FirebaseAuthException catch (e) {
      ...
      test('fails if creating a user if providing a weak password', () async {
        await ensureSignedIn(testEmail);
        try {
          await FirebaseAuth.instance.createUserWithEmailAndPassword(email: generateRandomEmail(), password: '1');
          fail('Should have thrown FirebaseAuthException');
        } on FirebaseAuthException catch (e) {
    ...
    group('fetchSignInMethodsForEmail()', () {
      test('should return password provider for an email address', () async {
        var providers = await FirebaseAuth.instance.fetchSignInMethodsForEmail(testEmail);
      ...
      test('should return empty array for a not found email', () async {
        var providers = await FirebaseAuth.instance.fetchSignInMethodsForEmail(generateRandomEmail());
    ...
    group('isSignInWithEmailLink()', () {
      test('should return true or false', () {
        ...
        expect(FirebaseAuth.instance.isSignInWithEmailLink(emailLink3), equals(false));
        expect(FirebaseAuth.instance.isSignInWithEmailLink(emailLink4), equals(true));
      });
    });
    ...
    group('sendPasswordResetEmail()', () {
      test('should not error', () async {
        ...
        try {
          await FirebaseAuth.instance.createUserWithEmailAndPassword(email: email, password: testPassword);
      ...
      test('fails if the user could not be found', () async {
        try {
          await FirebaseAuth.instance.sendPasswordResetEmail(email: 'does-not-exist@bar.com');
          fail('Should have thrown');
        } on FirebaseAuthException catch (e) {
    ...
    group('sendSignInLinkToEmail()', () {
      test('should send email successfully', () async {
        ...
        final oobCode = (await emulatorOutOfBandCode(email, EmulatorOobCodeType.emailSignIn))!;
    ...
    group('languageCode', () {
      ...
      test('should allow null value and default the device language code', () async {
        await FirebaseAuth.instance.setLanguageCode(null);

        expect(FirebaseAuth.instance.languageCode, isNotNull); // default to the device language or the Firebase projects default language
      }, skip: kIsWeb);
    ...
    group('signInWithCredential()', () {
      test('should login with email and password', () async {
        var credential = EmailAuthProvider.credential(email: testEmail, password: testPassword);
        await FirebaseAuth.instance.signInWithCredential(credential).then(commonSuccessCallback);
      });
      ...
      test('throws if login user is disabled', () async {
        ...
        try {
          ...
          expect(e.message, equals('The user account has been disabled by an administrator.'));
        } catch (e) {
          fail(e.toString());
        }
      });
      ...
      test('throws if login password is incorrect', () async {
        var credential = EmailAuthProvider.credential(email: testEmail, password: 'sowrong');
        try {
          await FirebaseAuth.instance.signInWithCredential(credential);
          fail('Should have thrown');
        } on FirebaseException catch (e) {
          expect(e.code, equals('wrong-password'));
          expect(e.message, equals('The password is invalid or the user does not have a password.'));
        } catch (e) {
          fail(e.toString());
        }
      });
      ...
      test('throws if login user is not found', () async {
        var credential = EmailAuthProvider.credential(email: generateRandomEmail(), password: testPassword);
        try {
          await FirebaseAuth.instance.signInWithCredential(credential);
          fail('Should have thrown');
        } on FirebaseException catch (e) {
          expect(e.code, equals('user-not-found'));
          expect(e.message, equals('There is no user record corresponding to this identifier. The user may have been deleted.'));
        } catch (e) {
          fail(e.toString());
        }
      });
    ...
    group('signInWithCustomToken()', () {
      test('signs in with custom auth token', () async {
        ...
        final customTokenUserCredential = await FirebaseAuth.instance.signInWithCustomToken(customToken);
    ...
    group('signInWithEmailAndPassword()', () {
      test('should login with email and password', () async {
        await FirebaseAuth.instance.signInWithEmailAndPassword(email: testEmail, password: testPassword).then(commonSuccessCallback);
      });
      ...
      test('throws if login password is incorrect', () async {
        try {
          await FirebaseAuth.instance.signInWithEmailAndPassword(email: testEmail, password: 'sowrong');
          fail('Should have thrown');
        } on FirebaseException catch (e) {
          expect(e.code, equals('wrong-password'));
          expect(e.message, equals('The password is invalid or the user does not have a password.'));
      ...
      test('throws if login user is not found', () async {
        try {
          await FirebaseAuth.instance.signInWithEmailAndPassword(email: generateRandomEmail(), password: testPassword);
          fail('Should have thrown');
        } on FirebaseException catch (e) {
          expect(e.code, equals('user-not-found'));
          expect(e.message, equals('There is no user record corresponding to this identifier. The user may have been deleted.'));
          ...
          unawaited(
            FirebaseAuth.instance.verifyPhoneNumber(
              ...
              verificationCompleted: (PhoneAuthCredential credential) {
                return completer.completeError(Exception('Should not have been called'));
              },
          ...
          unawaited(FirebaseAuth.instance.verifyPhoneNumber(
              ...
              verificationCompleted: (PhoneAuthCredential credential) {
                if (credential.smsCode != testSmsCode) {
                  return completer.completeError(Exception('SMS code did not match'));
                }
    ...
    group('tenantId', () {
      test('User associated with the tenantId correctly', () async {
        ...
        final userCredential = await FirebaseAuth.instance.signInWithEmailAndPassword(email: 'test-tenant@email.com', password: 'fake-password');
